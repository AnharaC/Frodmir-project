...
@user_private_router.message(
    or_f(Command("help", "start"), CommandStart(deep_link=True))
)
...
@user_private_router.message(
    StateFilter(None), or_f(Command("/survey"), (F.text.lower() == "опитування"))
)
async def command_start_handler(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo=image,
        caption="За якими ознаками ви хотіл би розпочати?",
...
async def detailed_survey(callback: types.CallbackQuery):
    ...
    await callback.message.answer_video(
        video=video, caption="Ой ой ой а це ще не доступно, соси бібу"
    )


## Все про очи
@user_private_router.callback_query(
    MessageState.quest_1, F.data.startswith("type_of_color_eye")
)
...
@user_private_router.callback_query(
    MessageState.analis_answer, F.data.startswith("final_yes_kari")
)
...
@user_private_router.callback_query(
    MessageState.analis_answer, F.data.startswith("final_no_kari")
)
...
@user_private_router.callback_query(
    MessageState.quest_2, F.data.startswith("green/light_brown")
)
async def analis_answer_green_light_brown(
...
## Все про групу крові
@user_private_router.callback_query(
    MessageState.quest_1, F.data.startswith("type_of_blood")
)
...
## Перша група крові
@user_private_router.callback_query(
    MessageState.quest_2, F.data.startswith("first_blood")
)
async def first_quest(callback: types.CallbackQuery, state: FSMContext):
...
## Друга група крові
@user_private_router.callback_query(
    MessageState.quest_2, F.data.startswith("second_blood")
)
async def first_quest(callback: types.CallbackQuery, state: FSMContext):
...
@user_private_router.callback_query(
    MessageState.analis_answer, F.data.startswith("final_yes_blood")
)
async def analis_answer(callback: types.CallbackQuery, state: FSMContext):
...
@user_private_router.callback_query(
    MessageState.analis_answer, F.data.startswith("final_no_blood")
)
async def analis_answer_no(callback: types.CallbackQuery, state: FSMContext):
...
## Терться група крові
@user_private_router.callback_query(
    MessageState.quest_2, F.data.startswith("third_blood")
)
async def first_quest(callback: types.CallbackQuery, state: FSMContext):
...
@user_private_router.callback_query(
    MessageState.quest_2, F.data.startswith("fourth_blood")
)
async def first_quest(callback: types.CallbackQuery, state: FSMContext):
