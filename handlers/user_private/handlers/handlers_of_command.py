...
from ..constants import ARTICLES_URL
...
@user_private_router.message(
    StateFilter(
        MessageState.quest_1,
        MessageState.quest_2,
        MessageState.quest_3,
        MessageState.st_quest_2,
        MessageState.st_quest_3,
        MessageState.stage_1,
        MessageState.stage_2,
    ),
...
async def block_commands(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Опа, а низя низя!! Доки не скасуешь опитування, або не пройдешь низя низя"
    )
...
async def command_articles_handler(message: Message, state: FSMContext):
    ...
    if response.status_code == 200:
        ...
        await message.answer(
            ...
            reply_markup=get_callback_btns(btns={"Відмінити": "cancel_articles"}),
        )
...
async def command_start_handler(message: Message, state: FSMContext):
    ...
    await message.answer_photo(
        photo=FSInputFile(image),
        caption="Оберіть ознаку за якою ви пройдете два опитування (Від себе і свого партнера): ",
...
async def back_step_handler(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if current_state == MessageState.quest_1:
        ...
        await callback.message.answer(
            "А все! Нема куди повертатися, анлак бро",
    ...
    await callback.message.answer(
        "Ви повернулись на попередне питання. Зачекайте поки повториться питання"
    )
