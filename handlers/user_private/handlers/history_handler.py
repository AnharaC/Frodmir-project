...
@user_private_router.message(StateFilter(MessageState.waiting_for_survey_selection), Command('help', 'start', 'about', 'survey'))
async def block_commands(message: Message, state: FSMContext):
    ...
    await message.answer("Опа, а низя низя!! Доки не скасуешь history, або не закінчешь роботу з histor низя буде використовувати команди")
...
async def detailed_survey(message: Message, state: FSMContext):
    ...
    await show_survey_history(message, survey_filenames, current_page, total_pages, state)

# Handler що показує кількість пройдених опитуваннь
async def show_survey_history(message: Message, survey_filenames: list, current_page: int, total_pages: int, state: FSMContext):
    ...
    await message.answer(history_message, reply_markup=get_callback_btns(btns={
        "⬅️ Назад": f"page_{current_page - 1}",
        "➡️ Вперед": f"page_{current_page + 1}",
        "❌ Відмінити": "cancel_history"
    }))

    await state.update_data(survey_filenames=survey_filenames, current_page=current_page, total_pages=total_pages)
...
async def handle_pagination(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data.startswith("page_"):
        ...
        await show_survey_history(callback.message, survey_filenames, new_page, total_pages, state)
...
@user_private_router.callback_query(StateFilter("*"), F.data.startswith("cancel_history"))
